...
import { BaseMessage, MessageUnion, SystemMessage, HumanMessage, AIMessage } from '@langchain/core/messages';
...
type EventType = Record<string, { messages: string[] }>;
...
for await (const event of await graph.stream({messages: messages1}, config1)) {
	for (const [node, values] of Object.entries(event as EventType)) {
...
function reduceMessages(left: any[], right: any[]): MessageUnion[] {
