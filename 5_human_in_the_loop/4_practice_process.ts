...
const logFile = fs.createWriteStream('result4.txt', { flags: 'a' });

// Redirect console.log to also write to the file
const originalConsoleLog = console.log;
console.log = function (...args) {
    originalConsoleLog(...args); // Keep the original behavior of console.log
    logFile.write(args.map(arg => (typeof arg === 'object' ? JSON.stringify(arg, null, 2) : arg)).join(' ') + '\n');
};
...
const stateIndex = states2.length - 3;
const saveState = await graph.getState(states2[stateIndex]);
...
// Now update the state again but set the node to Node1
