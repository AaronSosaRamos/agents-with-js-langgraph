...
for await (const event of await graph.stream({
	task: 'What is the Transformers Architecture for LLMs?',
	maxRevisions: 2,
	revision: 1
}, thread)) {
...
async function planNode(state: any) {
	const messages = [
...
async function researchPlanNode(state: any) {
	const messages = [
...
async function generationNode(state: any) {
...
async function reflectionNode(state: any) {
	const messages = [
...
async function researchCritiqueNode(state: any) {
	const messages = [
